...
function CpuDisplay({ hostname }) {
  const [data, setData] = useState([]);
  useEffect(() => {
    const fetch_ = () => fetch(`http://localhost:80/metric/cpu?hostname=${hostname}`)
      .then(r => r.json()).then(setData).catch(() => setData([]));
    fetch_();
    const t = setInterval(fetch_, 1000);
    return () => clearInterval(t);
...
function DiskDisplay({ hostname }) {
  const [data, setData] = useState([]);
  useEffect(() => {
    const fetch_ = () => fetch(`http://localhost:80/metric/disk?hostname=${hostname}`)
      .then(r => r.json()).then(setData).catch(() => setData([]));
    fetch_();
    const t = setInterval(fetch_, 1000);
    return () => clearInterval(t);
...
function OpenportsDisplay({ hostname }) {
  const [data, setData] = useState([]);
  useEffect(() => {
    const fetch_ = () => fetch(`http://localhost:80/metric/openports?hostname=${hostname}`)
      .then(r => r.json()).then(setData).catch(() => setData([]));
    fetch_();
    const t = setInterval(fetch_, 1000);
    return () => clearInterval(t);
...
function ProcessesDisplay({ hostname }) {
  const [data, setData] = useState([]);
  useEffect(() => {
    const fetch_ = () => fetch(`http://localhost:80/metric/processes?hostname=${hostname}`)
      .then(r => r.json()).then(setData).catch(() => setData([]));
    fetch_();
    const t = setInterval(fetch_, 1000);
    return () => clearInterval(t);
...
function ConnectionsDisplay({ hostname }) {
  const [data, setData] = useState([]);
  useEffect(() => {
    const fetch_ = () => fetch(`http://localhost:80/metric/connections?hostname=${hostname}`)
      .then(r => r.json()).then(setData).catch(() => setData([]));
    fetch_();
    const t = setInterval(fetch_, 1000);
    return () => clearInterval(t);
